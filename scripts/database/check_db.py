...
def check_database():
    ...
    try:
        ...
        if tasks:
            ...
            for task in tasks[:3]:
                print(
                    f"  - {task.key}: {task.summary[:50] if task.summary else 'N/A'}..."
                )
        ...
        if history:
            ...
            for entry in history[:3]:
                print(
                    f"  - Task ID: {entry.task_id}, Status: {entry.status}, Date: {entry.start_date}"
                )
        ...
        if sync_logs:
            ...
            for log in sync_logs[-3:]:
                print(
                    f"  - {log.sync_started_at}: {log.status} - {log.tasks_processed} задач"
                )
