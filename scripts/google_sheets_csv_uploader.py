    ...
    def process_files_with_markers(self) -> Dict[str, int]:
        ...
        for filename in files_with_markers:
            ...
            if success:
                ...
                stats['processed'] += 1
                logging.info(f"Successfully processed and removed marker for {filename}")
            else:
                self.file_monitor.mark_file_failed(filename, "Processing failed")
                stats['failed'] += 1
...
def main():
    ...
    parser.add_argument("--process-all", action="store_true", help="Process all unprocessed files")
    parser.add_argument("--process-markers", action="store_true", help="Process files with upload markers (from Telegram)")
    parser.add_argument("--process-file", type=str, help="Process specific CSV file")
    parser.add_argument("--monitor", action="store_true", help="Start continuous monitoring")
    parser.add_argument("--stats", action="store_true", help="Show statistics")
    parser.add_argument("--config", action="store_true", help="Show configuration")
    parser.add_argument("--cleanup", action="store_true", help="Cleanup old file records")
