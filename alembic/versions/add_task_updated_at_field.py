...
def upgrade() -> None:
    """Add task_updated_at field to tracker_tasks table."""
    op.add_column('tracker_tasks', sa.Column('task_updated_at', sa.DateTime(), nullable=True))
    
    # Create index for better performance on queries by task_updated_at
    op.create_index('idx_tracker_tasks_task_updated', 'tracker_tasks', ['task_updated_at'])
