...
def upgrade() -> None:
    ...
    # Create indexes for tracker_tasks (matching add_tracker_tables.py)
    op.create_index(
        "idx_tracker_tasks_tracker_id", "tracker_tasks", ["tracker_id"], unique=True
    )
    op.create_index(
        "idx_tracker_tasks_last_sync", "tracker_tasks", ["last_sync_at"], unique=False
    )
    ...
    # Create indexes for tracker_task_history (matching add_tracker_tables.py)
    op.create_index(
    ...
    # Create indexes for tracker_sync_logs (matching add_tracker_tables.py)
    op.create_index(
        "idx_tracker_sync_logs_status", "tracker_sync_logs", ["status"], unique=False
    )
    op.create_index(
        "idx_tracker_sync_logs_started",
        "tracker_sync_logs",
        ["sync_started_at"],
        unique=False,
    )
