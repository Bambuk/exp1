...
class Settings(BaseSettings):
    ...
    # Application
    APP_NAME: str = Field(default="Radiator CLI", json_schema_extra={"env": "APP_NAME"})
    ...
    # Email
    SMTP_HOST: str = Field(
        default="smtp.gmail.com", json_schema_extra={"env": "SMTP_HOST"}
    )
    ...
    EXTERNAL_API_KEY: str = Field(
        default="", json_schema_extra={"env": "EXTERNAL_API_KEY"}
    )
    EXTERNAL_API_URL: str = Field(
        default="", json_schema_extra={"env": "EXTERNAL_API_URL"}
    )

    # Yandex Tracker API
    TRACKER_API_TOKEN: str = Field(
        default="", json_schema_extra={"env": "TRACKER_API_TOKEN"}
    )
    ...
    TRACKER_MAX_WORKERS: int = Field(
        default=10, json_schema_extra={"env": "TRACKER_MAX_WORKERS"}
    )
    TRACKER_REQUEST_DELAY: float = Field(
        default=1.0, json_schema_extra={"env": "TRACKER_REQUEST_DELAY"}
    )
    TRACKER_SYNC_BATCH_SIZE: int = Field(
        default=100, json_schema_extra={"env": "TRACKER_SYNC_BATCH_SIZE"}
    )

    # Task limits - unified constants for all components
    DEFAULT_LARGE_LIMIT: int = Field(
        default=1000, json_schema_extra={"env": "DEFAULT_LARGE_LIMIT"}
    )
    DEFAULT_SEARCH_LIMIT: int = Field(
        default=100, json_schema_extra={"env": "DEFAULT_SEARCH_LIMIT"}
    )
    MAX_UNLIMITED_LIMIT: int = Field(
        default=1000000, json_schema_extra={"env": "MAX_UNLIMITED_LIMIT"}
    )
    API_PAGE_SIZE: int = Field(default=100, json_schema_extra={"env": "API_PAGE_SIZE"})

    # Reports Configuration
    REPORTS_DIR: str = Field(
        default="data/reports", json_schema_extra={"env": "REPORTS_DIR"}
    )
    TEST_REPORTS_DIR: str = Field(
        default="tests/test_reports", json_schema_extra={"env": "TEST_REPORTS_DIR"}
    )
