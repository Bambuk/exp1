...
from radiator.core.config import log_limit_info, settings
...
class TaskSearchCommand:
    ...
    def search_tasks(
        self, query: str, limit: int = None, output_format: str = "table"
    ) -> List[Dict[str, Any]]:
        """
    ...
    def format_output(
        self, tasks: List[Dict[str, Any]], output_format: str = "table"
    ) -> str:
    ...
    def run(self, query: str, limit: int = None, output_format: str = "table") -> bool:
        ...
        try:
            # Search for tasks
            tasks = self.search_tasks(
                query=query, limit=limit, output_format=output_format
            )
...
Examples:
  ...
  python -m radiator.commands.search_tasks 'Queue: CPO' --format json
    ...
    parser.add_argument(
        "query", help="Yandex Tracker search query (e.g., 'Queue: CPO Status: Open')"
    )
    ...
    success = search_cmd.run(
        query=args.query, limit=args.limit, output_format=args.format
    )
