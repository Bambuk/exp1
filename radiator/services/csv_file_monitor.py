...
class CSVFileMonitor:
        ...
        if self.state_file.exists():
            try:
                with open(self.state_file, 'r', encoding='utf-8') as f:
                    ...
                    logger.info(f"Loaded state: {len(self.known_files)} known files, {len(self.processed_files)} processed")
            ...
            if file_path.is_file() and file_path.suffix.lower() == '.csv':
                ...
                try:
                    ...
                    if file_size > GoogleSheetsConfig.MAX_FILE_SIZE:
                        logger.warning(f"File {file_path.name} is too large ({file_size / (1024*1024):.1f}MB), skipping")
                        continue
    ...
    def get_new_csv_files(self) -> List[str]:
        ...
        for filename, timestamp in current_files.items():
            ...
            is_modified = (filename in self.file_timestamps and 
                          timestamp > self.file_timestamps.get(filename, 0))
            
            if is_new or is_modified:
                new_files.append(filename)
                self.known_files.add(filename)
                self.file_timestamps[filename] = timestamp
                logger.info(f"Found {'new' if is_new else 'modified'} CSV file: {filename}")
    ...
    def get_stats(self) -> Dict[str, int]:
        ...
        return {
            ...
            'unprocessed_files': len([f for f in current_files.keys() if f not in self.processed_files])
        }
