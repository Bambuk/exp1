...
class GoogleSheetsService:
    ...
    def _get_group_value(self, row: Dict[str, Any]) -> str:
        """
        Get group value from row (team if available, otherwise author).

        Args:
            row: Row data dictionary

        Returns:
            Group value (team or author)
        """
    ...
    def _filter_valid_data(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Filter out rows with empty TTD or TTM values.

        Args:
            df: DataFrame with details data
        ...
        if "TTD" not in df.columns or "TTM" not in df.columns:
            return pd.DataFrame()

        # Keep only rows where both TTD and TTM are not null/empty
        valid_mask = df["TTD"].notna() & df["TTM"].notna()
        return df[valid_mask]

    def _create_pivot_data(
        self, df: pd.DataFrame, pivot_type: str
    ) -> List[Dict[str, Any]]:
        """
        Create pivot data from details DataFrame.
        ...
        Returns:
            List of dictionaries with pivot data
        """
        if df.empty:
            return []
        ...
        # Group by team/author and quarter
        grouped = valid_df.groupby(
            [valid_df.apply(lambda row: self._get_group_value(row), axis=1), "Квартал"]
        )
        ...
        for (group_name, quarter), group_data in grouped:
            if pivot_type == "ttd":
                ...
                pivot_data.append(
                    {
                        "Команда/Автор": group_name,
                        "Квартал": quarter,
                        "TTD Mean": round(ttd_values.mean(), 2),
                        "TTD P85": round(ttd_values.quantile(0.85), 2),
                        "TTD Count": len(ttd_values),
                        "TTD Pause Mean": round(ttd_pause_values.mean(), 2)
                        if len(ttd_pause_values) > 0
                        else 0,
                    }
                )
                ...
                pivot_data.append(
                    {
                        "Команда/Автор": group_name,
                        "Квартал": quarter,
                        "TTM Mean": round(ttm_values.mean(), 2),
                        "TTM P85": round(ttm_values.quantile(0.85), 2),
                        "TTM Count": len(ttm_values),
                        "Tail Mean": round(tail_values.mean(), 2)
                        if len(tail_values) > 0
                        else 0,
                        "Tail P85": round(tail_values.quantile(0.85), 2)
                        if len(tail_values) > 0
                        else 0,
                    }
                )
    ...
    def _create_pivot_sheet(
        self, sheet_id: str, data: List[Dict[str, Any]], sheet_name: str
    ) -> Optional[int]:
        """
        ...
        try:
            ...
            values = [headers] + [
                [str(row[header]) for header in headers] for row in data
            ]
            ...
            response = (
                self.service.spreadsheets()
                .batchUpdate(spreadsheetId=sheet_id, body=request_body)
                .execute()
            )
            ...
            # Auto-resize columns
            self._auto_resize_columns(sheet_name, len(headers))

            logger.info(f"Successfully created pivot sheet: {sheet_name}")
            return new_sheet_id

        except Exception as e:
            logger.error(f"Failed to create pivot sheet {sheet_name}: {e}")
            return None

    def create_pivot_tables_from_details(
        self, details_sheet_id: str
    ) -> Dict[str, Optional[int]]:
        """
        Create pivot tables from details sheet data.

        Args:
            details_sheet_id: Google Sheets document ID containing details data

        Returns:
            Dictionary with sheet IDs: {'ttd_pivot': sheet_id, 'ttm_pivot': sheet_id}
        """
        try:
            # Read details data from the sheet
            details_data = self._read_csv_file_from_sheet(details_sheet_id)
            if details_data is None or details_data.empty:
                logger.warning("No details data found for pivot tables")
