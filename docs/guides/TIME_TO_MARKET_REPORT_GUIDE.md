...
Команда `generate_ttm_details_report` генерирует детализированный CSV отчёт о времени выполнения CPO задач с полным набором метрик. Отчёт включает TTM, TTD, Tail, DevLT, паузы, время в статусах и возвраты.
