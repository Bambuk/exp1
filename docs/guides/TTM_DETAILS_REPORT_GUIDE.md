...
- **DevLT (Development Lead Time)** - время в статусе "МП / В работе" (только для валидных записей >= 5 минут)
...
- Время от первого перехода в done-статус до stable_done
- Показывает, сколько времени задача "болталась" между done и другими статусами

### DevLT (Development Lead Time)

- Время в статусе "МП / В работе"
- Учитываются только валидные записи (длительность >= 5 минут)
- Исключаются открытые интервалы (без end_date)
...
### Tail
- **Низкие значения** (0-2 дня): Стабильное завершение без возвратов
- **Высокие значения** (10+ дней): Множественные возвраты после первого done
