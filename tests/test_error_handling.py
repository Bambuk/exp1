...
class TestErrorHandling:
    ...
    def test_tracker_service_connection_error(self):
        ...
        with patch('radiator.services.tracker_service.logger'):
            ...
            with patch('radiator.services.tracker_service.requests.request', side_effect=ConnectionError("Connection failed")):
    ...
    def test_tracker_service_timeout_error(self):
        ...
        with patch('radiator.services.tracker_service.logger'):
            ...
            with patch('radiator.services.tracker_service.requests.request', side_effect=TimeoutError("Request timeout")):
                result = service.search_tasks("test query")
                assert result == []
    ...
    def test_sync_command_database_error(self):
        ...
        with patch('radiator.commands.sync_tracker.logger'):
            ...
            with patch('radiator.commands.sync_tracker.tracker_service') as mock_service:
                ...
                mock_service.get_tasks_batch.return_value = [("TEST-1", {"id": "TEST-1"})]
    ...
    def test_sync_command_partial_failure(self):
        ...
        with patch('radiator.commands.sync_tracker.logger'):
            ...
            with patch('radiator.commands.sync_tracker.tracker_service') as mock_service:
                ...
                mock_service.extract_task_data.return_value = {"id": "TEST-1", "key": "TEST-1", "summary": "Test Task", "status": "Done"}
                mock_service.get_changelogs_batch.return_value = [("TEST-1", [])]
                mock_service.extract_status_history.return_value = []
    ...
    def test_report_generation_data_service_error(self):
        ...
        with patch('radiator.commands.generate_time_to_market_report.logger'):
            ...
            cmd.data_service.get_tasks_for_period.side_effect = Exception("Data service error")
    ...
    def test_pause_time_calculation_edge_cases(self):
        ...
        invalid_history = [
            StatusHistoryEntry("New", "New", None, None),  # Invalid date
            StatusHistoryEntry("Приостановлено", "Приостановлено", datetime(2024, 1, 5, tzinfo=timezone.utc), None)
        ]
    ...
    def test_tail_metric_calculation_edge_cases(self):
        ...
        history = [
            StatusHistoryEntry("New", "New", datetime(2024, 1, 1, tzinfo=timezone.utc), None),
            StatusHistoryEntry("МП / Внешний тест", "МП / Внешний тест", datetime(2024, 1, 5, tzinfo=timezone.utc), None),
            StatusHistoryEntry("Done", "Done", datetime(2024, 1, 10, tzinfo=timezone.utc), None)
        ]
    ...
    def test_enhanced_group_metrics_calculation_edge_cases(self):
        ...
        # Test with empty times
        result = service.calculate_enhanced_group_metrics("TestGroup", [], [], [], [], [], [])
        assert result.group_name == "TestGroup"
        assert result.ttd_metrics.times == []
        assert result.ttm_metrics.times == []
        assert result.tail_metrics.times == []
        ...
        # Test with None times
        result = service.calculate_enhanced_group_metrics("TestGroup", None, None, None, None, None, None)
        assert result.group_name == "TestGroup"
        assert result.ttd_metrics.times == []
        assert result.ttm_metrics.times == []
        assert result.tail_metrics.times == []
