...
class TestInitialStatusFromChangelog:
    ...
    def test_extract_status_history_should_use_from_field_for_initial_status(self, mock_service):
        ...
        assert initial_entry["status"] == "Беклог", f"Initial status should be 'Беклог', got '{initial_entry['status']}'"
        assert initial_entry["status_display"] == "Беклог", "Should set status_display"
        assert initial_entry["start_date"] == task_data["created_at"], "Should use created_at as start_date"
        assert initial_entry["end_date"] == datetime(2024, 1, 1, 10, 0, 0, tzinfo=timezone.utc), "Should end when first change starts"
        
        # Second entry should be the first change
        change_entry = result[1]
        assert change_entry["status"] == "В работе", f"First change should be 'В работе', got '{change_entry['status']}'"
        assert change_entry["status_display"] == "В работе", "Should set status_display"
        assert change_entry["start_date"] == datetime(2024, 1, 1, 10, 0, 0, tzinfo=timezone.utc), "Should start when change occurred"
        assert change_entry["end_date"] is None, "Last status should have no end_date"

    def test_extract_status_history_should_handle_multiple_changes_correctly(self, mock_service):
        ...
        assert result[0]["end_date"] == datetime(2024, 1, 1, 10, 0, 0, tzinfo=timezone.utc)
        
        # Second entry: first change
        assert result[1]["status"] == "В работе"
        assert result[1]["start_date"] == datetime(2024, 1, 1, 10, 0, 0, tzinfo=timezone.utc)
        assert result[1]["end_date"] == datetime(2024, 1, 1, 15, 0, 0, tzinfo=timezone.utc)
        
        # Third entry: second change
        assert result[2]["status"] == "Готово"
        assert result[2]["start_date"] == datetime(2024, 1, 1, 15, 0, 0, tzinfo=timezone.utc)
        assert result[2]["end_date"] is None

    def test_extract_status_history_should_fallback_to_current_status_when_no_from_field(self, mock_service):
        ...
        assert result[0]["status"] == "Беклог", "Should use current status as fallback when 'from' is missing"
        assert result[0]["start_date"] == task_data["created_at"]
        assert result[0]["end_date"] == datetime(2024, 1, 1, 10, 0, 0, tzinfo=timezone.utc)
    ...
    def test_extract_status_history_should_handle_empty_changelog_with_current_status(self, mock_service):
        ...
        assert result[0]["status"] == "Беклог", "Should use current status when changelog is empty"
        ...
        changelog = [
            {
                ...
                "fields": [
                    {
                        "field": {"id": "assignee", "display": "Assignee"},  # ← НЕ статус
                        "from": {"display": "User1"},
                        "to": {"display": "User2"}
                    }
                ]
