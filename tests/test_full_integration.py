...
from radiator.commands.models.time_to_market_models import (
    StatusHistoryEntry, TimeMetrics, GroupMetrics, QuarterReport, TimeToMarketReport
)
...
class TestFullIntegration:
    ...
    def mock_database(self):
        ...
        mock_histories = {
            1: [
                StatusHistoryEntry("New", "New", datetime(2024, 1, 1, tzinfo=timezone.utc), None),
                StatusHistoryEntry("In Progress", "In Progress", datetime(2024, 1, 3, tzinfo=timezone.utc), None),
                StatusHistoryEntry("Готова к разработке", "Готова к разработке", datetime(2024, 1, 5, tzinfo=timezone.utc), None),
                StatusHistoryEntry("Done", "Done", datetime(2024, 1, 10, tzinfo=timezone.utc), None)
            ],
            2: [
                StatusHistoryEntry("New", "New", datetime(2024, 1, 2, tzinfo=timezone.utc), None),
                StatusHistoryEntry("In Progress", "In Progress", datetime(2024, 1, 4, tzinfo=timezone.utc), None),
                StatusHistoryEntry("Готова к разработке", "Готова к разработке", datetime(2024, 1, 7, tzinfo=timezone.utc), None),
                StatusHistoryEntry("Done", "Done", datetime(2024, 1, 15, tzinfo=timezone.utc), None)
            ]
        }
    ...
    def test_complete_time_to_market_report_generation(self, mock_database):
        ...
        with patch('radiator.commands.generate_time_to_market_report.logger'):
            ...
            cmd.config_service.load_status_mapping.return_value = mock_database['status_mapping']
            
            # Mock data service
            cmd.data_service = Mock()
            cmd.data_service.get_tasks_for_period.return_value = mock_database['tasks']
            cmd.data_service.get_task_history.side_effect = lambda task_id: mock_database['histories'].get(task_id, [])
        ...
        with patch('radiator.commands.sync_tracker.logger'):
            ...
            with patch('radiator.commands.sync_tracker.tracker_service') as mock_service:
                ...
                mock_service.get_tasks_batch.return_value = [
                    ("TEST-1", {"id": "TEST-1", "key": "TEST-1", "summary": "Test Task 1", "status": "Done"}),
                    ("TEST-2", {"id": "TEST-2", "key": "TEST-2", "summary": "Test Task 2", "status": "Done"})
                ]
                mock_service.extract_task_data.return_value = {"id": "test", "key": "TEST-1", "summary": "Test Task", "status": "Done"}
                mock_service.get_changelogs_batch.return_value = [("TEST-1", []), ("TEST-2", [])]
                ...
                with patch('radiator.commands.sync_tracker.TrackerSyncCommand') as mock_sync:
                    with patch('radiator.commands.sync_tracker.tracker_task_history') as mock_history_crud:
                        with patch('radiator.commands.sync_tracker.tracker_sync_log') as mock_sync_log_crud:
                            ...
                            mock_task_crud.bulk_create_or_update.return_value = {"created": 2, "updated": 0}
                            ...
                            with patch.object(cmd.db, 'add'):
                                with patch.object(cmd.db, 'commit'):
                                    with patch.object(cmd.db, 'refresh'):
                                        ...
                                        mock_service.get_tasks_by_filter.assert_called_once_with({}, limit=10)
    ...
    def test_complete_search_tasks_workflow(self, mock_database):
        ...
        with patch('radiator.commands.search_tasks.logger'):
            ...
            # Mock tracker service
            with patch('radiator.commands.search_tasks.tracker_service') as mock_service:
    ...
    def test_end_to_end_workflow(self, mock_database):
        ...
        with patch('radiator.commands.sync_tracker.logger'), \
             patch('radiator.commands.generate_time_to_market_report.logger'), \
            ...
            ttm_cmd.config_service.load_quarters.return_value = mock_database['quarters']
            ttm_cmd.config_service.load_status_mapping.return_value = mock_database['status_mapping']
            
            ttm_cmd.data_service = Mock()
            ttm_cmd.data_service.get_tasks_for_period.return_value = mock_database['tasks']
            ttm_cmd.data_service.get_task_history.side_effect = lambda task_id: mock_database['histories'].get(task_id, [])
            ...
            sc_cmd.config_service.load_status_mapping.return_value = mock_database['status_mapping']
            
            sc_cmd.data_service = Mock()
            sc_cmd.data_service.get_tasks_for_period.return_value = mock_database['tasks']
            sc_cmd.data_service.get_task_history.side_effect = lambda task_id: mock_database['histories'].get(task_id, [])
    ...
    def test_performance_integration(self, mock_database):
        ...
        with patch('radiator.commands.generate_time_to_market_report.logger'):
            ...
            cmd.data_service.get_task_history.side_effect = lambda task_id: large_histories.get(task_id, [])
            ...
            if report.quarters and "Q1 2024" in report.quarter_reports:
                ...
                for i in range(10):
                    ...
                    assert author_metrics.total_tasks == 10  # 100 tasks / 10 authors = 10 tasks per author
