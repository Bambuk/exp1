...
class TestLimitConsistency:
    ...
    def test_limit_propagation_through_layers(self):
        ...
        with patch('radiator.services.tracker_service.TrackerAPIService') as mock_service_class:
            mock_service = Mock()
    ...
    def test_unlimited_mode_handling(self):
        ...
        with patch('radiator.services.tracker_service.TrackerAPIService') as mock_service_class:
            mock_service = Mock()
            mock_service_class.return_value = mock_service
            mock_service.search_tasks.return_value = ["task" + str(i) for i in range(1, 101)]
    ...
    def test_large_limit_handling(self):
        ...
        with patch('radiator.services.tracker_service.TrackerAPIService') as mock_service_class:
            mock_service = Mock()
            mock_service_class.return_value = mock_service
            mock_service.search_tasks.return_value = ["task" + str(i) for i in range(1, 1001)]
            ...
            mock_service.search_tasks.assert_called_with("test query", limit=large_limit)
            assert isinstance(result, list)
